...
| `projection` | 由 aggregate 或 user_config 确定性派生的视图投影（可重建、不产生新事实） |
| `user_config` | 用户显式配置的输入（不是 domain 事实） |
| `phase4` | 依赖 Issue #4 冻结后可得（Mastery / Review Scheduling v0.1） |
...
> 以下所有字段名均为**规划占位**。最终字段名与状态枚举由 Issue #4 的正式契约决定。
>
> **Phase 4 当前真实状态**：P4.6 / P4.7 的测试骨架已进入 main（`docs/PHASE4_TEST_MATRIX.md`、`data/review/fixture-schema.draft.json`、`scripts/validate_review.py`），但均为 `draft` / `PENDING_CONTRACT_FREEZE`。测试骨架不提供任何 UI 可消费字段。

| UI Metric | UI Meaning | Domain Field | Domain Source | Source Category | Current Availability | Null Semantics | Explain Source | Notes / Constraints |
|---|---|---|---|---|---|---|---|---|
| mastery | 某 review item 的掌握状态 | `TBD — dependent on Issue #4` | MasteryReviewState v0.1 | `phase4` | `Phase 4`（不存在） | 无 evidence → `TBD`（预计为 `new` 或显式 `insufficient_evidence`，由 Issue #4 决定） | mastery policy version + transition reason + evidence + `as_of` | **accuracy ≠ mastery**；不得由 topic accuracy 阈值直接生成；不得由 UI 推断 |
| mastery 分布 | 各状态的 item 数量 | `TBD — dependent on Issue #4` | MasteryReviewState | `phase4` | `Phase 4` | 无 item → `0`（真实事实） | 状态计数 | 用于 `OperationalStatsGrid` T8 |
| 到期复习数 `due_count` | 截至 `as_of` 到期的 item 数量 | `TBD — dependent on Issue #4` | MasteryReviewState（scheduling projection） | `phase4` | `Phase 4` | 无 item → `0`；无 policy → 不可用 | review policy version + `as_of` | 是 item 数量还是事件计数由 Issue #4 冻结 |
| 下次到期时间 `review_due_at` | 该 item 下次到期时刻 | `TBD — dependent on Issue #4` | MasteryReviewState | `phase4` | `Phase 4` | 无 evidence → `null` | interval transition + policy version | instant / local date / 二者并存的语义由 Issue #4 冻结 |
| 复习间隔 `review_interval` | 当前 policy 下的间隔 | `TBD — dependent on Issue #4` | Review policy | `phase4` | `Phase 4` | 未进入调度 → `null` | policy version + transition reason | 具体间隔序列由 Issue #4 裁决；**当前 `1 / 3 / 7 / 15` 只是草案候选** |
| 上次复习时间 `last_review_at` | 最近一次 review evidence 的时刻 | `TBD — dependent on Issue #4` | MasteryReviewState | `phase4` | `Phase 4` | 无 review → `null` | review evidence 事件 | 首次学习与 review 的区分由 Issue #4 冻结 |
| 逾期状态 overdue | 已过 `review_due_at` 的状态 | `TBD — dependent on Issue #4` | scheduling projection | `phase4` | `Phase 4` | 无 due → 不适用 | `review_due_at` + `as_of` | 与 mastery 是**两个独立维度**；见 `REVIEW_MASTERY_UX.md` |
| transition reason | 为什么状态 / 到期时间变成现在这样 | `TBD — dependent on Issue #4` | Mastery / Review policy | `phase4` | `Phase 4` | engine 未输出 → UI 显示「原因不可用」 | policy 输出 | **UI 不得生成推测性解释** |
| review item 身份 | 复习对象的稳定 id | `TBD — dependent on Issue #4` | Review Model | `phase4` | `Phase 4` | 不适用 | 身份构成规则 | 不得依赖展示名称 / 题干 / 自由文本；迁移策略未冻结（D8） |
| review item 粒度 | item kind | `TBD — dependent on Issue #4`（候选 Topic / Question / Case Capability） | Review Model | `phase4` | `Phase 4` | 不适用 | Review Model 契约 | UI 必须显示 `item_kind`，不得混用同一进度条 |
| mastery / review policy 版本 | 解释可复现所需的 policy 标识 | `TBD — dependent on Issue #4` | Mastery / Review policy | `phase4` | `Phase 4` | 未冻结 → 不适用 | policy 元数据 | 进入 `PolicyVersionFooter` |
