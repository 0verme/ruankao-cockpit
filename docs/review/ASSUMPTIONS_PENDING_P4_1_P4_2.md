...
| A2 | outcome primitive 为 `success` / `failure` / `insufficient` 或等价表示。 | Evidence 使用 `supported`、`insufficient_evidence`、`unsupported`。`insufficient_evidence` 明确不是 failure；`success/failure` 不写入 Evidence。综合题的 `correct` 由 adapter 无歧义映射，score 类 evidence 使用 `review-outcome/raw-facts/v0.1` 的 `insufficient` 保守映射。 | RECONCILED_WITH_ADAPTER（threshold/rubric 仍开放） |
...
| A7 | score 是否等价于 failure/success 由 Evidence contract 的版本化映射决定。 | Evidence 只保存 score pair；`review-outcome/raw-facts/v0.1` 保守映射为 `insufficient`，没有冻结阈值、rubric 或采分点 outcome 规则，不伪造 success/failure。 | RECONCILED（threshold/rubric 仍为后续 adapter 的 BLOCKING_DECISION） |
...
## BLOCKING_DECISION：score evidence 的后续 policy outcome adapter
...
3. P4.5 已采用独立、版本化的 `review-outcome/raw-facts/v0.1` 保守 adapter：记录适用 item kind、版本和解释；该映射不能回写到 Evidence record，也不能由 AI 产生。
4. case/capability raw score 保留在 Evidence trace，并显式视为 policy `insufficient`；不得静默视为 failure/zero。score threshold/rubric 仍是后续 adapter version 的 BLOCKING_DECISION。

这不是把 P4.3/P4.4 规则写回 Evidence；adapter 只把已冻结的 raw facts 安全接入 `PolicyEvidence`，并把版本与 reason 写入 replay 输出。
